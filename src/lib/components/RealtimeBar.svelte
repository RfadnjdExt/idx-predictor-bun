<script>
  ...
  let dot      = $derived(STATUS_COLOR[status.split(':')[0].split(' ')[0]] ?? '#3d5a78');
  let isLive   = $derived(status === 'connected');
  let isFailed = $derived(status.startsWith('failed') || status.startsWith('error'));
...
<div class="bar">
  ...
  {:else if isFailed}
    <!-- Error / failed — tunjukkan pesan dan tombol retry -->
    <div class="err-row">
  ...
  {:else}
    ...
    <div class="status-row">
      <span class="dot spin-dot" class:spinning={!isLive} style="background:{dot}"></span>
...
.status-txt { font-family:monospace; font-weight:700; color:var(--bull); }
.err-txt    { font-family:monospace; font-size:11px; color:var(--bear); max-width:220px;
              white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }

.spinning   { animation:pulse 1s ease infinite; }
